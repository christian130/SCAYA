...
var MultiUpload = new Class(
{
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			var delete_button = new Element(
				'img',
				{
					'src':'../../../imagenes/seguridad/iconos/error.png',
					'alt':'Delete',
					'title':'Delete',
					'events':{
						'click':function( uid ){
			...
			alert( 'No puede cargar mas de ' + this.max + ' archivos'  );
		}
	...
	deleteRow:function( uid ){
		...
		if( confirm( 'Esta seguro que desea eliminar el siguiente elemento de la lista?\r\n\n	' + deleted_row.element.value ) ){
