<?php
/*class clsConnex{
	var $manejador = "postgresql";
	var $servidor = "127.0.0.1";
	var $usuario = "secausr";
	var $clave = "123seca@lc";
	var $bd = "AlcSeca";
	var $schema = "public";
}*/
class clsConnex{
	...
	var $usuario = "cbiometrico";
	var $clave = "biometrico";
	var $bd = "cbiometrico";
	var $schema = "public";
}
...
class Recordset
{
	...
	function __construct(){
		$initParams = new clsConnex();
		$this->manejador = $initParams->manejador;
		$this->servidor = $initParams->servidor;
		$this->usuario = $initParams->usuario;
		$this->clave = $initParams->clave;
		$this->bd = $initParams->bd;
		$this->schema = $initParams->schema;
	}
	
	//Funcion con la que se abre el recordset conectandose a la base de datos
	function abrir()
	{
		switch ($this->manejador)
		{
			...
			case "mssql":
				...
				$this->tabla = @mssql_query ($this->sql) or die("<strong>".htmlentities("Wrror en la consulta: ".$this->sql."!")."</strong>");
			...
			case "postgresql":
				if ($this->limite_cantidad > 0)
				{
					$this->sql = $this->sql." OFFSET ".$this->limite_inicio." LIMIT ".$this->limite_cantidad;
				}
				$this->conexion = @pg_pconnect("host=" . $this->servidor . " user=" . $this->usuario . " password=" . $this->clave . " dbname=" . $this->bd) or die("<strong>".htmlentities("Servidor, usuario o clave incorecta!")."</strong>");
				@pg_query("SET search_path TO " . $this->schema);
	...
	//Funcion que libera la memoria del recordset
	function cerrar()
	{
		switch ($this->manejador)
		{
	...
	//Funcion que permite desplazar el puntero del recordset
	function desplazar()
	{
	...
	//Funcion que permite mover el cursor al registro siguiente dentro del recordset
	function siguiente()
	{ 
	...
	//Funcion que permite mover el cursor al registro anterior dentro del recordset
	function anterior()
	{
	...
	//Funcion para desplazarse a un registro cualquiera segun la posici�n dentro del recordset
	...
	//Funcion para desplazarse al principio del recordset
	...
	//Funcion para desplazarse al final del recordset
	...
	{
		if($this->manejador == "mysql" || $this->manejador == "postgresql")
		{
			$this->abrir();
			if($this->total_registros > 0)
			{
	...
	//Funcion que permite crear el link Primera Pagina del paginador
	//Solo trabaja con el motor de base de datos Mysql
	...
	//Funcion que permite crear el link Anterior Pagina del paginador
	//Solo trabaja con el motor de base de datos Mysql
	...
	//Funcion que permite crear el link Siguiente Pagina del paginador
	//Solo trabaja con el motor de base de datos Mysql
	...
	//Funcion que permite crear el link ultima pagina del paginador
	//Solo trabaja con el motor de base de datos Mysql
	...
	//Funcion que permite crear todo el paginador
	...
	//Funcion que permite llenar un ComboBox en HTML
	...
	//Funcion que permite llenar un grupo de RadioButton en HTML
	...
	//Funcion que permite optimizar una tabla
	function optimizartabla($tabla)
	{
		switch ($this->manejador)
		{
			case "mysql":	
				...
				break;
			case "postgresql":
				@pg_pconnect("host=" . $this->servidor . " user=" . $this->usuario . " password=" . $this->clave . " dbname=" . $this->bd) or die("<strong>".htmlentities("�Servidor, usuario o clave incorecta!")."</strong>");
				$sql = "OPTIMIZE TABLE $tabla";
				@pg_query($sql);
				break;
		}
	...
	//Funcion que permite truncar una tabla
	...
	//Funcion que permite dar formato correcto a las fechas en MySQL
	...
	//Funcion que permite dar formato moneda 
	function moneda($valor)
	{
		return "Bs. ".number_format($valor,2,",",".");
	}
	
	//Funcion que permite dar formato numero
	function montos($valor)
	{
